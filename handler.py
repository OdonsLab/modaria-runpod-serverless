...
def queue_prompt(workflow, client_id):
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"ComfyUI rechazó el workflow ({response.status_code}): {response.text}"
        )

    data = response.json()

    if "error" in data:
        raise RuntimeError(f"Error de ComfyUI al encolar: {data['error']}")
...
def wait_for_completion(prompt_id):
    ...
    while True:
        if time.time() - start_time > WORKFLOW_TIMEOUT:
            raise TimeoutError(
                f"El workflow {prompt_id} superó el timeout de {WORKFLOW_TIMEOUT}s"
            )
        ...
        if response.status_code == 200:
            ...
            if prompt_id in history:
                ...
                if status.get("status_str") == "error":
                    raise RuntimeError(f"El workflow falló en ComfyUI: {status}")
                ...
                for msg_type, msg_data in status.get("messages", []):
                    if msg_type == "execution_error":
                        raise RuntimeError(f"Error de ejecución: {msg_data}")
...
def handler(job):
    ...
    if not workflow:
        return {"status": "error", "message": "Falta 'workflow' en el input del job"}

    client_id = job_input.get("client_id", str(uuid.uuid4()))

    try:
        prompt_id = queue_prompt(workflow, client_id)
        print(f"Workflow encolado con prompt_id={prompt_id}")
